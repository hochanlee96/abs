...
### 시뮬레이션 로직
1. **능력치 반영**: 투수(Stuff, Velocity, Control) vs 타자(Contact, Power, Speed)의 대결을 LLM 프롬프트에 반영합니다.
2. **끝장 승부**: 무승부 없이 승패가 결정될 때까지 경기(연장전)가 지속됩니다.
3. **재귀 제한**: 긴 경기를 처리하기 위해 LangGraph `recursion_limit`을 1000으로 설정했습니다.
