...
@app.get("/me")
def me(db: Session = Depends(get_db), payload: dict = Depends(verify_google_id_token_from_header)):
...
async def ws_match(websocket: WebSocket, match_id: int):
    ...
    try:
        # Decode URL encoded token if necessary, but usually query params are decoded by FastAPI
        # However, the frontend sends "dev:jiewan", which might be encoded as "dev%3Ajiewan"
        # FastAPI decodes query params automatically.
        
        # Call verify_token directly (not the dependency wrapper)
        from .auth_google import verify_token
        verify_token(token)
    except Exception:
